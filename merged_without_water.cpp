...
bool inTextSegment = true, inDataSegment = false;
...
unordered_map<string, string> funct3_map = {
    ...
    {"sw", "010"}, {"beq", "000"}, {"bne", "001"}, {"jal", "000"}, {"sh","001"},{"lui","NULL"},{"auipc", "NULL"}, {"sb","000"},{"jalr", "000"},{"bge","101"},{"blt","100"}
};
...
string convert_to_machine_code(vector<string> tokens) {
    ...
    if (type == "R") {  // R-format
        ...
        imm = bitset<13>(stoi(tokens[3])).to_string();  // 13-bit for branch offset
        return imm[0] + imm.substr(2, 6) + rs2 + rs1 + funct3 + imm.substr(8, 4) + imm[1] + opcode;
    }
    else if (type == "U") {
        rd = register_map[tokens[1]];  // Destination register
        int imm_value = stoi(tokens[2]);  // Convert immediate to int
    
        // Extract upper 20 bits
        imm_value = imm_value << 12;
       
        imm = bitset<32>(imm_value).to_string();  // Convert to 20-bit binary
        ...
        rd = register_map[tokens[1]];
        imm = bitset<21>(stoi(tokens[2])).to_string();  // 21-bit for jump offset
...
void processAssemblyFile(const string& inputFile, const string& outputFile) {
    ...
    while (getline(asmFile, line)) {
        ...
        if (inDataSegment) processDataLine(line, mcFile);
        else {
            ...
            string machine_code = convert_to_machine_code(tokens);
            if (!machine_code.empty()) {
                mcFile << "0x" << hex << currentTextAddress << " 0x" << bitset<32>(stoi(machine_code, 0, 2)).to_ulong()
                       << " , " << line << endl;
                currentTextAddress += 4;
            }
        }
    }
    asmFile.close();
